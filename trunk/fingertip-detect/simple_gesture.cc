 ...
 * either License.
 *
 * THIS FILE HAS BEEN MODIFIED FROM THE SOURCE AND ADAPTED FOR FINGERTIP
 * DETECTION.
 *
...
void DrawGLScene()
{
	...
	int numTips = feature_extractor->GetNumFingerTips();
	FeatureExtractor::VectorSegment* fingerSegs = feature_extractor->GetFingerVectors();
	for (int i = 0; i < numTips; i++) {
		glBegin(GL_LINES);
		glColor3f(255.0f, 255.0f, 0.0f);
		glLineWidth(5.0f);
		glVertex2f(fingerSegs[i].start%WIDTH, fingerSegs[i].start/WIDTH);
		glVertex2f(fingerSegs[i].end%WIDTH, fingerSegs[i].end/WIDTH);
		glEnd();
	}
...
void depth_cb(freenect_device *dev, void *v_depth, uint32_t timestamp)
{
	...
	// Compute absolute pixel distance and distance to closest pixel.
